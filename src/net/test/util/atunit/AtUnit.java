...
import net.test.chapter18_javaIoSystem.ProcessFiles;
...
public class AtUnit implements ProcessFiles.Strategy
{
    ...
    static class TestMethods extends ArrayList<Method>
    {
        void addIfTestMethod(Method m)
        {
            ...
            if (!(m.getReturnType().equals(boolean.class) ||
            m.getReturnType().equals(void.class))) throw new RuntimeException("@Test method" + " must return boolean or void");
    ...
    private static Method checkForCreatorMethod(Method m)
    {
        ...
        if (!m.getReturnType().equals(testClass)) throw new RuntimeException("@TestObjectCreate " + "must return instance of Class to be tested");
        if ((m.getModifiers() & Modifier.STATIC) < 1) throw new RuntimeException("@TestObjectCreate " + "must be static.");
        m.setAccessible(true);
        return m;
    }
    ...
    private static Method checkForCleanupMethod(Method m)
    {
        ...
        if (!m.getReturnType().equals(void.class)) throw new RuntimeException("@TestObjectCleanup " + "must return void");
        if ((m.getModifiers() & Modifier.STATIC) < 1) throw new RuntimeException("@TestObjectCleanup "  + " must be static.");
        if (m.getParameterTypes().length == 0 ||
                m.getParameterTypes()[0] != testClass) throw new RuntimeException("@TestObjectCleanup must take an argument of the tested type.");
        m.setAccessible(true);
        return m;
    }
    ...
    private static Object createTestObject(Method creator)
    {
        if (creator != null)
        {
            try {
                ...
                throw new RuntimeException("@TestObject (creator) method.");
            }
        }
        else
        {
